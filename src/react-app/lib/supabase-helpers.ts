...
export async function createEstudiantesBulk(estudiantes: any[]) {
    ...
    if (courseIds.length > 0) {
        ...
        courses?.forEach((c: any) => {
            courseMap.set(c.id, `${c.nombre_curso}${c.seccion ? ` - SecciÃ³n ${c.seccion}` : ''}`)
        })
    }
...
export async function getBoletinData(estudianteId: number) {
    ...
    // Handle PÃ¡rvulo I levels
    if (['PÃ¡rvulo I', 'Parvulo I', 'PÃ¡rvulo 1', 'Parvulo 1'].includes(cursoNombre) && estudiante.nivel_parvulo) {
